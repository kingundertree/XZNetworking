//
//  UIDevice+XZIdentifierAddition.m
//  XZMusic
//
...
@implementation UIDevice (XZIdentifierAddition)
...
- (NSString *)XZNet_uuid
{
    ...
    if (uuid.length == 0) {
        [[NSUserDefaults standardUserDefaults] setObject:[self XZNet_createUUID] forKey:key];
...
- (NSString *) XZNet_udid
{
    NSString *udid = [[XZUDIDGenerator sharedInstance] udid];
    if (udid.length==0) {
        udid = [self XZNet_uuid];
        [[XZUDIDGenerator sharedInstance] saveUDID:udid];
    }
    return udid;
}
...
- (NSString *)XZNet_macaddressMD5{
    ...
    if (macid.length == 0) {
        NSString *macaddress = [[UIDevice currentDevice] XZNet_macaddress];
